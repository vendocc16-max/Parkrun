 ...
 * - Feature flag: RATE_LIMIT_ENABLED env var (default: disabled)
 *
 * Expected behavior:
 * 1. First request from IP/email: { allowed: true, remaining: 5 }
 * 2. After 10 requests from same IP: { allowed: false, retryAfter: 60 }
 * 3. If rate limit is disabled: All requests allowed
 * 4. If the DB check fails: Fails open (allows request)
 */

import { checkRateLimit } from '../rate-limit'

describe('Rate Limit Middleware', () => {
  // To exercise the limiter locally, set RATE_LIMIT_ENABLED=true and use Supabase env vars.
